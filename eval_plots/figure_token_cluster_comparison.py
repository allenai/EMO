...
def add_rounded_card(ax, x, y, w, h, facecolor, edgecolor, lw=1.0,
                     rounding=0.025):
...
def render_cluster_card(ax, clusters, total_tokens, spec, top_n):
    ...
    ax.text(0.05, 0.91, spec["label"],
            fontsize=18, fontweight="bold", color=FG,
            transform=ax.transAxes, ha="left", va="top",
            family="sans-serif")
    ax.text(0.05, 0.862, spec["sublabel"],
            fontsize=10, color=FG_MUTED,
            transform=ax.transAxes, ha="left", va="top",
            family="sans-serif")

    # Tagline
    ax.text(0.95, 0.89, spec["subtitle"],
            fontsize=10.5, color=accent_dark, style="italic",
    ...
    ax.text(0.05, 0.806, "TOP CLUSTERS  ·  BY TOKEN COUNT",
            fontsize=8, color=FG_FAINT, fontweight="bold",
            transform=ax.transAxes, ha="left", va="top",
            family="sans-serif")
    ...
    for i, c in enumerate(sorted_clusters):
        ...
        # Cluster ID
        ax.text(0.105, y, f"C{c['id']:02d}",
                fontsize=7, color=FG_FAINT,
    ...
    ax.text(0.5, 0.022,
            ...
            fontsize=7.5, color=FG_FAINT, style="italic",
...
def render_doc_zoom_card(ax, tokens, spec, cluster_label, target_pct):
    ...
    card = FancyBboxPatch(
        ...
        facecolor=BG_CARD, edgecolor=BORDER, linewidth=0.9,
        zorder=0,
    )
    ax.add_patch(card)
    ...
    ax.text(
        chars_per_line + 0.4, header_y,
        f"{spec['label']}   ·   {target_pct:.0f}% of doc",
        fontsize=10, color=FG_MUTED,
        ha="right", va="center", family="sans-serif", zorder=3,
    )

    # Divider line between header and document
    ax.plot(
        [0.5, chars_per_line - 0.5], [max_lines + 0.1, max_lines + 0.1],
...
def main():
    ...
    gs = fig.add_gridspec(
        ...
        left=0.03, right=0.97, top=0.905, bottom=0.035,
    )
    ...
    for col_idx, (key, spec) in enumerate(MODELS):
        ...
        render_doc_zoom_card(ax, tokens=tokens, spec=spec,
                              cluster_label=cluster_label, target_pct=pct)
    ...
    fig.text(
        ...
        fontsize=11, color=FG_MUTED, style="italic",
        ha="center", va="top", family="sans-serif",
    )
    ...
    fig.text(
        ...
        fontsize=10, color=FG_MUTED, style="italic",

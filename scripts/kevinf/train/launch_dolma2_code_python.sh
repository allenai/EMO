#!/bin/bash
# Launch OLMo3-1B continued pretraining on dolma2-code-python (stack-edu Python only)
#
# Usage: bash scripts/kevinf/train/launch_dolma2_code_python.sh
#
# Model: OLMo3-1B (1B parameters)
# Data: dolma2-code-python (stack-edu Python, ~74.9B tokens)
...
for lr in 5e-5; do
  runname="train-olmo3-1b-${dataset}-${train_tokens_B}B-lr${lr}-eval-on-python-only"
  if [ -n "$load_path" ]; then
    runname="${runname}-ctd"
  fi
  ...
  python -m olmo_core.launch.beaker \
    ...
    --trainer.callbacks.lm_evaluator.eval_dataset.mix=dolma2-code-python \

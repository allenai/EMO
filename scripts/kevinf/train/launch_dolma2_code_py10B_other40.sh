#!/bin/bash
# Launch OLMo3-1B continued pretraining on dolma2-code-py10B-other40
#
# Usage: bash scripts/kevinf/train/launch_dolma2_code_py10B_other40.sh
#
# Model: OLMo3-1B (1B parameters)
# Data: dolma2-code-py10B-other40 (stack-edu 9 langs, 369.69B total)
#   Python = 20.26% => ~10.13B Python + ~39.87B other over 50B training
#   Same Python files as dolma2-code-python (controlled comparison)
# Base: 130B Dolma pretrained checkpoint

# Hyperparameters (easy to change for sweeps)
dataset="dolma2-code-py10B-other40"
warmup_fraction=0.1
train_tokens_B=50  # in billions

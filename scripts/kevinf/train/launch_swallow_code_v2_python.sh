#!/bin/bash
# Launch OLMo3-1B continued pretraining on swallow-code-v2-python
#
# Usage: bash scripts/kevinf/train/launch_swallow_code_v2_python.sh
#
# Model: OLMo3-1B (1B parameters)
# Data: swallow-code-v2-python (tokyotech-llm swallow-code-v2 stage5_python_medium, ~49.95B tokens)
#   Comparison against dolma2-code-python (stack-edu Python) to test data source quality
# Base: 130B Dolma pretrained checkpoint

# Hyperparameters (easy to change for sweeps)
dataset="swallow-code-v2-python"
warmup_fraction=0.1

#!/bin/bash
# Launch OLMo3-1B continued pretraining on dolma2-code-java (stack-edu Java only)
#
# Usage: bash scripts/kevinf/train/launch_dolma2_code_java.sh
#
# Model: OLMo3-1B (1B parameters)
# Data: dolma2-code-java (stack-edu Java, ~65.2B tokens)
...
for lr in 5e-5; do
  runname="train-olmo3-1b-${dataset}-${train_tokens_B}B-lr${lr}-eval-on-java-only"
  if [ -n "$load_path" ]; then
    runname="${runname}-ctd"
  fi

#!/bin/bash
# Launch OLMo3-1B continued pretraining on sponge-code-prose-p75 (web pages with code+prose, quality_p75+)
#
# Usage: bash scripts/kevinf/train/launch_sponge_code_prose_p75.sh
#
# Model: OLMo3-1B (1B parameters)
# Data: sponge-code-prose-p75 (sponge_211 code+prose web crawl, quality_p75 through p95, ~17.3B tokens available)
...
for lr in 5e-5; do
  ...
  python -m olmo_core.launch.beaker \
    ...
    --trainer.callbacks.lm_evaluator_2.eval_dataset.mix=sponge-code-prose-p75 \
    --trainer.callbacks.lm_evaluator_2.eval_dataset.mix_base_dir=s3://ai2-llm \
    --trainer.callbacks.lm_evaluator_2.name=lm_sponge_code_prose \

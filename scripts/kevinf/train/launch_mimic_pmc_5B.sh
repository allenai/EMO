#!/bin/bash
# Launch OLMo3-1B continued pretraining on mimic-iv-note + PMC mix (~5B tokens)
#
# Usage: bash scripts/kevinf/train/launch_mimic_pmc_5B.sh
#
# Model: OLMo3-1B (1B parameters)
# Data: mimic-iv-note (all, ~1.70B tokens) + PMC oa_comm (~3.34B tokens)
# Total: ~5.04B tokens
# Base: 130B Dolma pretrained checkpoint

# Hyperparameters (easy to change for sweeps)
dataset="mimic-pmc-5B"
warmup_fraction=0.1
train_tokens_B=5  # in billions

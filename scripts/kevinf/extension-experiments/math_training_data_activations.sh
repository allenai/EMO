...
gantry run \
    ...
    -- \
    bash -c "PYTHONPATH=. python -u src/scripts/eval/launch_logits_training.py --model ${MODEL_PATH} --mix ${MIX} --mix-base-dir ${MIX_BASE_DIR} --output-dir ${OUTPUT_DIR} --batch-size ${BATCH_SIZE} --seq-length ${SEQ_LENGTH}"

...
bash scripts/clustering/generate_pretraining_mix.sh

# 1. Extract logits from pretraining data (1M tokens, 100 tok/doc)
bash scripts/clustering/extract_pretraining.sh

# 2. Derive probs from logits
bash scripts/clustering/transform.sh \
    claude_outputs/clustering/pretraining/<model> probs

# 3. Cluster
bash scripts/clustering/cluster.sh \
    claude_outputs/clustering/pretraining/<model>

# 4. Visualize
bash scripts/clustering/visualize.sh \
...
bash scripts/clustering/extract_pretraining.sh [MODEL_PATH] [TARGET_TOKENS] [MAX_TOKENS_PER_DOC]

# MMLU (all 57 subjects, validation split)
bash scripts/clustering/extract_mmlu.sh [MODEL_PATH]

# HellaSwag (all splits)
bash scripts/clustering/extract_hellaswag.sh [MODEL_PATH]
```
...
```bash
bash scripts/clustering/transform.sh <DATA_DIR> <DERIVE>
```
...
bash scripts/clustering/cluster.sh <DATA_DIR>

# Custom config
bash scripts/clustering/cluster.sh <DATA_DIR> doc_topk_freq mean_pca_l2 spherical_kmeans 32

# Sweep over k values
bash scripts/clustering/sweep.sh <DATA_DIR>
```
...
```bash
bash scripts/clustering/visualize.sh <CLUSTER_DIR>
```

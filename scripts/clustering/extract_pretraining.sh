...
#
#   # Specific model
#   bash scripts/clustering/extract_pretraining.sh models/moereducedp512_1b14b_lr-4e-3_lb-1e-1_0211/step30995-hf
#
#   # Use specific GPUs
#   CUDA_VISIBLE_DEVICES=0,1 bash scripts/clustering/extract_pretraining.sh
set -euo pipefail
...
if [ ! -f "$COMPOSITION_FILE" ]; then
    ...
    echo "Run: bash scripts/clustering/generate_pretraining_mix.sh"
    exit 1
fi

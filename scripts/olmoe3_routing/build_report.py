...
def build_q4():
    ...
    body += card("info", "Method",
        ...
        "<b>STE</b> lets the language-model loss itself push d up when the experts just inside the pool are useful; "
        "<b>coverage</b> pushes d up while the experts just outside the pool still receive a noticeable share of the document's routing "
        "(the loss card below gives both exactly).</li>"
    ...
    body += card("info", "The loss",
        ...
        "them over about T ranks around d. <b>STE</b>: inside the language-model loss, each selected expert's weight is multiplied by its "
        "m<sub>e</sub>, so the loss can move d (the forward pass still uses the hard pool). <b>Coverage</b>: the share outside the pool is "

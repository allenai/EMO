...
def build_q4():
    ...
    body += card("info", "Method",
        ...
        "<li><b>Let pools grow where needed.</b> Something has to push back, or every pool shrinks to 16. Recall that the pool is the "
        "document's d highest-scoring experts, so the experts at ranks near d are the ones a slightly larger pool would add. Two choices of signal:"
        "<ul>"
        "<li><b>STE</b> asks the language-model loss. With a hard cut at rank d the loss cannot say whether a bigger pool would help. So during "
        "training the cut is blurred: an expert's weight in the pool goes from 1 to 0 gradually over the <b>T</b> ranks around d instead of "
        "jumping. Through those weights the loss can tell whether the experts just inside the cut are useful, and d moves accordingly.</li>"
        "<li><b>Coverage</b> asks the router. Averaging the router's softmax over a document's tokens gives each expert's share of that "
        "document's routing (say 3% for one expert, 0.5% for another; the shares sum to 100%). The coverage loss is the share left outside the pool, "
        "weighted by <b>&lambda;<sub>cov</sub></b>. Sort the experts by share, p<sub>1</sub> &ge; p<sub>2</sub> &ge; &hellip;; growing the pool from "
        "d to d+1 lowers the coverage loss by &lambda;<sub>cov</sub> &middot; p<sub>d+1</sub> and raises the size penalty by &lambda;<sub>d</sub> / 496, "
        "so it pays off exactly when p<sub>d+1</sub> &gt; &lambda;<sub>d</sub> / (&lambda;<sub>cov</sub> &middot; 496). Since the shares are sorted, "
        "the loss is lowest when the pool holds every expert above that threshold and no others; at the setting used below (&lambda;<sub>d</sub> = "
        "&lambda;<sub>cov</sub> = 1) the threshold is 1/496 &asymp; 0.2% of the document's routing. Documents that concentrate their routing on a few experts get small pools, "
        "documents that spread it get large ones, and the head learns to predict that cut from the hidden state.</li>"
        "</ul></li>"

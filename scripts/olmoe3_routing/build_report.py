...
def build_q4():
    ...
    body += card("info", "Method",
        ...
        "document's routing (say 3% for one expert, 0.5% for another; the shares sum to 100%). Each expert added to the pool brings in its share, "
        "weighted by <b>&lambda;<sub>cov</sub></b>, and costs a fixed amount of size penalty, &lambda;<sub>d</sub> / 496. So the pool ends up "
        "holding every expert whose share is above &lambda;<sub>d</sub> / (&lambda;<sub>cov</sub> &middot; 496), and no others; at the setting "
        "used below that is 0.2% of the document's routing. Documents that concentrate their routing on a few experts get small pools, "

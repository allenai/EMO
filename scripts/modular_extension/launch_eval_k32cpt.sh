#!/usr/bin/env bash
# Launch one 8-GPU Beaker job that computes per-cluster held-out CE for ONE checkpoint
# (eval_k32cpt_ce.py): 8 per-GPU processes, 4 clusters each. Idempotent per output file.
#
#   CKPT=/weka/.../models_v2/emo_64exp_50b_wsd_lr2e-3/step23842 TAG=step23842_100B \
#       bash scripts/modular_extension/launch_eval_k32cpt.sh
#   SNAPSHOT=/weka/.../pool/snapshots/pool_after_c05.pt CONFIG_FROM=/weka/.../step23842 \
#       TAG=carry_after_c05 bash scripts/modular_extension/launch_eval_k32cpt.sh
#
...
if [ -n "${SNAPSHOT:-}" ]; then
    : "${CONFIG_FROM:?set CONFIG_FROM (step dir with config.json) when using SNAPSHOT}"
...
for g in $(seq 0 7); do
    ...
    inner+="[ -f ${out} ] || CUDA_VISIBLE_DEVICES=${g} PYTHONPATH=.:src python -u scripts/modular_extension/eval_k32cpt_ce.py ${SRC_ARGS} --clusters ${lo}-${hi} --out ${out} > /results/eval_gpu${g}.log 2>&1 & "
done
...
python -m olmo_core.launch.beaker \
    ...
    -- bash -c "$inner"
echo "Launched eval job modext-k32ce-${TAG}"

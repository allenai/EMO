...
#     Beta(4, 1)-shaped distribution over the same [16, 512] range instead of uniformly:
#         u ~ U(0, 1);  d = round(16 + (512 - 16) * u ** (1 / 4))
#     (mean pool 413 vs 264 for uniform). Eval pool stays 512 (all experts).
#     Implemented by OLMOE3_EMO_POOL_DIST=beta:4 in scripts/sparse_experts/olmoe3_275m.py, which
#     patches EmoRouterV2._pool_sizes in-process (the pinned external/OLMo-core is unchanged).
#
#     Checkpoints: /weka/oe-training-default/ryanwang/EMO/debug_validation/olmoe3_275m_emo_beta4_10b
...
#     [pretraining, debug_validation, olmoe3_275m, 512e, emo, pool_beta4, jupiter, 10b].
#
#   git add ... && git commit && git push origin <branch>   # gantry clones from origin!
#   bash scripts/debug_validation/model_scripts/olmoe3_275m_emo_beta4_10b.sh [dry_run]
##############################################################
export OLMOE3_EMO=1
export OLMOE3_EMO_POOL_DIST=beta:4
export OLMOE3_EXPERIMENT=debug_validation
export OLMOE3_RUNNAME=olmoe3_275m_emo_beta4_10b

...
def build_overview(ser):
    ...
    findings = (
        ...
        "2.974 for the standard router: +0.036 over uniform EMO, so roughly 40% of EMO's total cost over the standard router comes from "
        "the pool restriction itself and 60% from the pools being random rather than relevance-chosen. Keeping the pool at least an eighth "
        "of the experts (d in [64, 512]) recovers half of that: 3.019 (+0.019 over uniform EMO). On the 300B held-out sample the "
        "[16, 512] arm scores 2.514 and the [64, 512] arm 2.498 against 2.465 for uniform EMO and 2.446 for the standard router "
        "(+0.048 and +0.033), slightly larger gaps than on the validation sets.</li>"

...
#       16 experts, 50B-token run, 1 shared expert, lb 1e-1, OLMoE-mix-0824, 8 nodes / 64 GPUs).
#       Identical to stdmoe_64exp_50b_wsd_lr2e-3.sh EXCEPT num_experts=16 (vs 64). top_k stays 8
#       (inherited from the base config, NOT scaled by num_experts), so active compute is unchanged
#       (8 routed of 15 standard + 1 shared); only TOTAL capacity shrinks -> the lowest-capacity
#       baseline, sitting furthest below 128e. Purpose: widen the lower->upper gap for the
#       extension experiments so upcycling's benefit is more visible than in the small 64e->128e gap.
...
#   MODE=beaker bash scripts/models_v2/stdmoe_16exp_50b_wsd_lr2e-3.sh
##############################################################
...
runname="stdmoe_16exp_50b_wsd_lr2e-3"

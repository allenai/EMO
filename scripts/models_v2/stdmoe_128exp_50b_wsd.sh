# PARENT: "scripts/models_v2/stdmoe_64exp_50b_wsd_lr2e-3.sh" + "scripts/models_v2/stdmoe_128exp_50b.sh"
# DESCRIPTION:
#     - A pure WSD STABLE TRUNK (standard top-k MoE, model type moe_lbreducedp_sharedexp,
#       128 experts, 50B-token run, 1 shared expert, lb 1e-1, OLMoE-mix-0824, 8 nodes / 64 GPUs).
#       Same WSD-stable-trunk recipe as stdmoe_64exp_50b_wsd_lr2e-3.sh / _lr4e-4.sh, but with
#       128 experts and the DEFAULT peak LR 4e-3:
#         1. Peak LR is 4e-3 (the recipe default).
...
#            scripts/models_v2/launch_wsd_decay.sh (which auto-extracts the 4e-3 peak LR from the

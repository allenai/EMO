...
# DESCRIPTION:
#     - Standard top-k MoE (model type moe_lbreducedp_sharedexp) with 128 experts, trained
#       as a TRUE 50B-token run: the LR cosine schedule decays directly over 50B
#       (max_duration=50B, no hard_stop), i.e. "what if we only ever had 50B tokens."
#       1 shared expert, lr 4e-3, lb 1e-1, OLMoE-mix-0824, 8 nodes / 64 GPUs.
#       Same recipe as stdmoe_64exp_50b.sh but with the full 128-expert pool. NOTE: this
#       differs from stdmoe_1b14b_50bof130b.sh (also 128 experts) in the LR horizon ---
#       that one uses a 130B schedule hard-stopped at 50B (LR only partially decayed),
#       whereas this one decays fully over 50B.

...
def saturation_gate(
    output: str, epoch: int, previous_epoch: int | None, log_path: str
) -> list[str]:
...
def build_chain(args: argparse.Namespace) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:
    ...
    endpoint.MODEL_ROOT = (
        old_root if (args.canonical_output or args.decay_embeddings) else OUTPUT_ROOT
    )
    ...
    try:
        ...
        for target in targets:
            ...
            if target == resume_from_predecay_epoch:
                ...
                stage_spec["tasks"][0]["arguments"][2] = shell.replace(checkpoint_guard, "", 1)
            if target == 1 and args.retry_failed:
                shell = stage_spec["tasks"][0]["arguments"][2]
                output_guard = f"test '!' -e {output}\n"
                if output_guard not in shell:
                    raise RuntimeError(
                        "failed retry spec is missing the expected output-root guard"
                    )
                stage_spec["tasks"][0]["arguments"][2] = shell.replace(output_guard, "", 1)
    ...
    for target, stage_spec in zip(active_targets, stage_specs):
        ...
        if args.stop_on_saturation and target != targets[-1]:
            stage_commands.extend(saturation_gate(output, target, previous_target, log_path))
        runtime_stages.append(
            {
                "epoch": target,
                "shell": "\n".join(
                    (runtime_prelude + stage_commands[1:]) if not runtime_stages else stage_commands
                ),
                "stopDecision": (
                    f"{output}/.embwd_e{target}.decision"
                    if args.stop_on_saturation and target != targets[-1]
                    else None
                ),
            }
...
def register_submission(
    ...
    if args.decay_embeddings:
        ...
        if not any(column.get("key") == column_key for column in report.get("columns", [])):
...
def configure_failed_retry(args: argparse.Namespace) -> None:
    ...
    if completed_frontier == 0:
        # A container-start failure leaves no frontier; replay E1 in the guarded
        # canonical directory after the caller has confirmed the failed attempt.
        return

#!/usr/bin/env python3
"""Submit the authorized two-node 153M Pool-3B E384-E448 continuation."""
...
import run_dense_small_pool3b_bs256_e448_continuation as runner
...
def verify_ready(record: dict[str, Any]) -> None:
    if 384 not in {int(x) for x in record.get("resolvedCheckpointEpochs", [])}:
        raise RuntimeError("E384 PD is not registered as retained")
    if "384" not in record.get("postDecayResults", {}):
        raise RuntimeError("matched E384 POST result is unresolved")
    ...
    if Path(str(record.get("output"))) != runner.EXPECTED_OUTPUT:
        raise RuntimeError("registered output differs from the exact E384 lineage")
...
def register(report: dict[str, Any], experiment: str, revision: str) -> None:
    ...
    record.setdefault("experimentHistory", []).append({
        ...
        "maxValidatedEpoch": 384, "output": record.get("output"),
        "stoppedAt": datetime.now(tz=UTC).isoformat(),
    })
    record.update({
        "experiment": experiment, "revision": revision, "policy": runner.POLICY,
        "status": "submitted", "beakerStatus": "submitted", "currentEpoch": 388,
        ...
        "continuationSourceEpoch": 384, "continuationTargetEpoch": 448,
        "continuationCheckpointEpochs": list(runner.CHECKPOINT_EPOCHS),
        "evaluationEpochs": [448], "checkpointIntervalEpochs": 4,
        "checkpointCleanupKeepEpochs": [256, 320, 384, 448],

#!/usr/bin/env python3
"""Continue exact 153M Pool-3B E448 PD to E512 on two 8-GPU nodes."""
...
POLICY = "dense_small_pool3b_bs256_e512_integrated_v1"
DEFAULT_MANIFEST = Path("scripts/models/manifests/dense-small-pool3b-bs256-e512-continuation-v1.json")
EXPECTED_ID = "dense-153m-dclm3b-bs256-lr2e-3-wd0.1"
SOURCE_EPOCH = 448
TARGET_EPOCH = 512
CHECKPOINT_EPOCHS = tuple(range(452, 513, 4))
...
STATE_DIR = EXPECTED_OUTPUT / ".constant_checkpoint_producer_pool3b_e448_e512_integrated_v1"
...
def validate(config: dict[str, Any], item: dict[str, Any], *, filesystem: bool) -> None:
    ...
    if filesystem:
        ...
        if not checkpoint_complete(source, 16):
            raise FileNotFoundError(f"incomplete exact E448 source {source}")
        ...
        if not all(checks):
            raise RuntimeError("E448 source does not preserve the selected DR+WT+EmbedWD recipe")
...
def torchrun(name: str, arguments: list[str], log_path: Path, stage: str) -> None:
    ...
    if nodes != 2:
        raise RuntimeError("E512 continuation requires exactly two nodes")
    if rank == 0:
...
def producer_arguments(config: dict[str, Any], item: dict[str, Any], resume: Path, pending: list[int]) -> list[str]:
    ...
    name = f"{EXPECTED_ID}-constant-pd-e448-e512-v1"
    return [
        *producer.base_arguments(item, str(config["repeatedManifest"])), "--dynamic-repacking",
        ...
        "--trainer.callbacks.wandb.tags=[pretraining,step1,0802,pool3b-repeat,dense-153m,dr_wt_embwd,bs256,lr2e-3,wd0.1,constant-lr,pre-decay,e448-e512-continuation,two-node]",
...
def run(config: dict[str, Any], item: dict[str, Any]) -> None:
    ...
    if pending:
        ...
        if is_leader():
            ...
            print(f"DENSE_POOL3B_E512_PD_START id={EXPECTED_ID} source_epoch={resume_epoch} target_epoch=512", flush=True)
        torchrun(f"{EXPECTED_ID}-constant-pd-e448-e512-v1", producer_arguments(config, item, resume, pending), STATE_DIR / "producer.log", "pd")
    target = EXPECTED_OUTPUT / f"step{checkpoint_step(TARGET_EPOCH)}"
    if not checkpoint_complete(target, 16):
        raise RuntimeError(f"producer exited without exact E512 checkpoint {target}")
    if is_leader():
        print(f"DENSE_POOL3B_INTEGRATED_PD_RETAINED id={EXPECTED_ID} epoch=512 checkpoint={target}", flush=True)

    result_path = evaluator.state_dir(item, str(EXPECTED_OUTPUT)) / "results" / "e512.json"
    if not result_path.is_file():
        post_output = evaluator.state_dir(item, str(EXPECTED_OUTPUT)) / "post_decay_runs" / "e512"
        endpoint_step = producer.total_step(TARGET_EPOCH, producer.TARGET_POOL_TOKENS, 256)
        endpoint = post_output / f"step{endpoint_step}"
        name = f"{EXPECTED_ID}-post-e512-v1"
        log_path = evaluator.state_dir(item, str(EXPECTED_OUTPUT)) / "logs" / "e512.log"
        if endpoint.is_dir():
        ...
        if is_leader():
            print(f"DENSE_SMALL_CHECKPOINT_EVALUATOR_START id={EXPECTED_ID} epoch=512 source={target} output={post_output}", flush=True)
        torchrun(name, args, log_path, "post")
        if is_leader():
            if not endpoint.is_dir():
                raise RuntimeError(f"E512 decay exited without endpoint {endpoint}")
    ...
    if is_leader():
        previous = json.loads((evaluator.state_dir(item, str(EXPECTED_OUTPUT)) / "results" / "e448.json").read_text())
        saturated = Decimal(str(result["validationExact"])) >= Decimal(str(previous["validationExact"]))
        decision = {
            "policy": POLICY, "status": "saturated" if saturated else "terminal_e512",
            "comparisonGroup": "post_decay_only", "criterion": "strict_non_improvement",
            "epochs": [448, 512], "validationExact": {"448": previous["validationExact"], "512": result["validationExact"]},
            "producerStoppedAfterEpoch": 512, "nextProducerEpoch": None, "hardTerminalEpoch": 512,
        }
        for epoch in CHECKPOINT_EPOCHS:
            ...
            if checkpoint_complete(path, 16):
                shutil.rmtree(path)
        producer.atomic_json(STATE_DIR / "decisions" / "e512.json", decision)
        print(f"DENSE_POOL3B_INTEGRATED_POST_RESULT id={EXPECTED_ID} epoch=512 json={json.dumps(result,separators=(',',':'),sort_keys=True)}", flush=True)
        print(f"DENSE_POOL3B_INTEGRATED_DECISION id={EXPECTED_ID} epoch=512 json={json.dumps(decision,separators=(',',':'),sort_keys=True)}", flush=True)
        print(f"DENSE_POOL3B_INTEGRATED_JOB_COMPLETE id={EXPECTED_ID} terminal_epoch=512 status={decision['status']}", flush=True)
...
def main() -> None:
    ...
    if args.validate_only:
        print(f"validated {EXPECTED_ID} E448-E512 two-node continuation")
        return
    run(config, item)

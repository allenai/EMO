#!/usr/bin/env bash
# DESCRIPTION:
#     - Runs one target-specific WSD endpoint on the uniformly sampled 0802
#       unique 5B training-only DCLM pool.
...
runname="dense_1b_step2_0802_unique_dclm5b_wsd_e${epochs}_lr${lr}_wd${wd}_warmup${warmup_steps}${run_suffix}"
...
launch src/scripts/train/olmo2-1B.py "${runname}" \
		...
		--trainer.callbacks.wandb.tags="[pretraining, step2, 0802, unique-data, dclm-train-only, document-disjoint, uniform-document-sample, wsd, warmup24]" \

...
NAME = "dense-153m-dclm3b-bs256-lr4e-3-wd0.1-saturation-protected-two-node-v1"
MIN_RUNTIME = "8h"
...
def build_spec(item: dict[str, Any], revision: str, priority: str) -> dict[str, Any]:
    ...
    spec["description"] = (
        ...
        "POST at E32/E64/E96/E128/E160/E192/E224/E256/E320/E384; stop on first adjacent "
...
def register(experiment: str, revision: str) -> None:
    ...
    if any(record.get("id") == runner.EXPECTED_ID for record in records):
...
def main() -> None:
    ...
    report = json.loads(REPORT.read_text())
    if any(record.get("id") == runner.EXPECTED_ID for record in report.get("pool3bLearningRateProbes", [])):
        raise RuntimeError("Pool-3B LR4e-3 probe is already registered")

#!/usr/bin/env python3
"""Resume exact 1B Pool-3B BS128 E80 PD to E96 on two 8-GPU nodes."""
...
import run_dense_1b_pool3b_bs128_e65_e80_two_node as previous
import run_dense_constant_checkpoint_producer as producer

POLICY = "dense_1b_pool3b_bs128_e80_e96_two_node_integrated_v1"
DEFAULT_MANIFEST = Path("scripts/models/manifests/dense-1b-pool3b-bs128-e80-e96-two-node-v1.json")
...
STATE_DIR = EXPECTED_OUTPUT / ".dense_1b_pool3b_bs128_e80_e96_two_node_v1"
...
def validate(config: dict[str, Any], item: dict[str, Any], *, filesystem: bool) -> None:
    ...
    if previous.checkpoint_world_size(source) != 16:
        raise FileNotFoundError(f"incomplete exact 16-rank E80 checkpoint {source}")
    ...
    if not all(checks):
        raise RuntimeError("E80 source does not preserve the selected DR+WT+EmbedWD recipe")
...
def producer_arguments(item: dict[str, Any], resume: Path, pending: list[int]) -> list[str]:
    ...
    name = f"{EXPECTED_ID}-constant-pd-e80-e96-two-node-v1"
    return [
        *producer.base_arguments(item),
        ...
        "--trainer.callbacks.wandb.tags=[pretraining,step1,0802,pool3b-repeat,dense-1b,dr_wt_embwd,bs128,lr1e-3,wd0.3,constant-lr,pre-decay,e80-e96-continuation,two-node]",
        ...
        "--dynamic-repacking", "--force_exact_trainer_load_path=true",
        f"--trainer.load_path={resume}", "--trainer.load_trainer_state=true",
        "--trainer.load_optim_state=true", "--trainer.reset_data_loader_state_on_load_path=false",
...
def cleanup() -> list[int]:
    preserve = {32, 48, 64, 80, 96}
    removed: list[int] = []
    for epoch in CHECKPOINT_EPOCHS:
        if epoch in preserve:
            continue
        checkpoint = EXPECTED_OUTPUT / f"step{checkpoint_step(epoch)}"
        if checkpoint.exists():
            if previous.checkpoint_world_size(checkpoint) != 16:
...
def run(config: dict[str, Any], item: dict[str, Any]) -> None:
    ...
    complete = [
        epoch for epoch in range(SOURCE_EPOCH, TARGET_EPOCH + 1)
        if previous.checkpoint_world_size(EXPECTED_OUTPUT / f"step{checkpoint_step(epoch)}") is not None
    ]
    ...
    if pending:
        ...
        if is_leader():
            producer.atomic_json(STATE_DIR / "producer.json", {
                ...
                "resumeEpoch": resume_epoch, "resumeCheckpoint": str(resume),
                "targetEpoch": TARGET_EPOCH, "pendingEpochs": pending,
                "nodeCount": 2, "gpuCount": 16, "rankMicrobatchSequences": 8,
                "gradientAccumulationSteps": 1,
            })
            print(f"DENSE_POOL3B_E96_TWO_NODE_PD_START id={EXPECTED_ID} source_epoch={resume_epoch} source={resume} target_epoch=96", flush=True)
        torchrun(f"{EXPECTED_ID}-constant-pd-e80-e96-two-node-v1", producer_arguments(item, resume, pending), STATE_DIR / "producer.log", "pd")
    target = EXPECTED_OUTPUT / f"step{checkpoint_step(TARGET_EPOCH)}"
    if previous.checkpoint_world_size(target) != 16:
        raise RuntimeError(f"producer exited without exact complete 16-rank E96 checkpoint {target}")
    if is_leader():
        print(f"DENSE_POOL3B_INTEGRATED_PD_RETAINED id={EXPECTED_ID} epoch=96 checkpoint={target}", flush=True)

    result_path = original.post_result_path(item, TARGET_EPOCH)
    if not result_path.is_file():
        output = evaluator.state_dir(item) / "post_decay_runs" / "e96"
        endpoint_step = producer.total_step(TARGET_EPOCH, int(item["poolTokens"]), int(item["batchSequences"]))
        endpoint = output / f"step{endpoint_step}"
        name = f"{EXPECTED_ID}-postdecay-e96-two-node-v1"
        if endpoint.is_dir():
            log_path = evaluator.state_dir(item) / "logs" / "postdecay_e96_two_node_recovered_eval.log"
            arguments = evaluator.evaluation_arguments(item, endpoint, output / "eval-two-node", f"{name}-recovered-eval")
        else:
            log_path = evaluator.state_dir(item) / "logs" / "postdecay_e96_two_node.log"
            arguments = evaluator.postdecay_arguments(item, TARGET_EPOCH, target, output, name)
        if is_leader():
            print(f"DENSE1B_CHECKPOINT_EVALUATOR_START id={EXPECTED_ID} epoch=96 source={target} output={output}", flush=True)
        torchrun(name, arguments, log_path, "post")
        if is_leader():
            if not endpoint.is_dir():
                raise RuntimeError(f"E96 decay exited without endpoint {endpoint}")
            result = evaluator.parse_result(log_path, item, TARGET_EPOCH, endpoint, target)
            result.update({"policy": POLICY, "producerPolicy": POLICY, "source": "integrated_two_node_wsd_decay_and_heldout_eval"})
            producer.atomic_json(result_path, result)
    result = json.loads(result_path.read_text()) if is_leader() else wait_for(result_path)
    if is_leader():
        previous_result = original.load_post_result(item, SOURCE_EPOCH)
        decision = {
            "policy": POLICY, "status": "terminal_e96", "comparisonGroup": "post_decay_only",
            "criterion": "evaluate_authorized_e96_and_stop", "epochs": [SOURCE_EPOCH, TARGET_EPOCH],
            "validationExact": {"80": float(previous_result["validationExact"]), "96": float(result["validationExact"])},
            "producerStoppedAfterEpoch": TARGET_EPOCH, "nextProducerEpoch": None,
            "hardTerminalEpoch": TARGET_EPOCH, "removedRecoveryEpochs": cleanup(),
        }
        producer.atomic_json(original.state_dir() / "decisions" / "e96.json", decision)
        print(f"DENSE_POOL3B_INTEGRATED_POST_RESULT id={EXPECTED_ID} epoch=96 json={json.dumps(result, separators=(',', ':'), sort_keys=True)}", flush=True)
        print(f"DENSE_POOL3B_INTEGRATED_DECISION id={EXPECTED_ID} epoch=96 json={json.dumps(decision, separators=(',', ':'), sort_keys=True)}", flush=True)
        print(f"DENSE_POOL3B_INTEGRATED_JOB_COMPLETE id={EXPECTED_ID} terminal_epoch=96 status=terminal_e96", flush=True)
...
def main() -> None:
    ...
    if args.validate_only:
        print(f"validated {EXPECTED_ID} E80-E96 two-node continuation")
        return
    run(config, item)

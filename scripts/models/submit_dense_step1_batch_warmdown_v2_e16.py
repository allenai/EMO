#!/usr/bin/env python3
"""Submit the gated Dense-1B Version-2 BS64 E12 -> E16 continuation."""
...
SOURCE_EXPERIMENT = "01M00TGBT6273XD7T6QBNRRET2"
...
OUTPUT_NAME = (
    "bs64_dr_lr1e-3_wd0.3_init=bs64e12_lr1e-03_wd0.3_"
    "init=bs256e8_lr1e-03_wd0.333_init=bs1024e4_lr1e-03_wd0.333"
)
SOURCE_STEP = 17_517
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--name", default="dense-1b-batch-warmdown-v2-e16"
    )
...
def extract_source_args(spec: dict[str, Any]) -> list[str]:
    ...
    raise RuntimeError("could not extract the completed Version-2 BS64 arguments")
...
def continuation_args(source_args: list[str]) -> list[str]:
    ...
    replacements = (
        ...
        (
            ...
            "--trainer.callbacks.wandb.tags="
            "[pretraining,step1,batch-warmdown,v2,e16,dr,bs64]",
        ),
        (
...
def build_spec(
    ...
    commands = [
        "set -euo pipefail",
        f"test -d {shlex.quote(SOURCE)} || {{ echo {shlex.quote('V2_E16_PREFLIGHT_ERROR missing source: ' + SOURCE)} >&2; exit 40; }}",
        f"test -f {shlex.quote(TRAIN_MANIFEST)} || exit 41",
        f"test -f {shlex.quote(VALIDATION_MANIFEST)} || exit 42",
        f"if compgen -G {shlex.quote(OUTPUT + '/step*')} > /dev/null; then echo {shlex.quote('V2_E16_PREFLIGHT_ERROR output checkpoint exists: ' + OUTPUT)} >&2; exit 43; fi",
        f'test "$(git rev-parse HEAD)" = "{args.revision}" || {{ echo "V2_E16_PREFLIGHT_ERROR revision mismatch" >&2; exit 44; }}',
        shlex.join(
            [
                "echo",
                "BATCH_WARMDOWN_V2_E16_PREFLIGHT source_step=17517 source_epoch=12 "
...
def validate(spec: dict[str, Any]) -> None:
    ...
    required = (
        SOURCE,
        OUTPUT,
        "source_step=17517 source_epoch=12 added=15259 pre_decay=29497 end=32776",
        "--trainer.max_duration={value: 32776, unit: steps}",
        "--trainer.callbacks.checkpointer.fixed_steps=[29497, 32776]",
        "--trainer.load_trainer_state=true",
        "--trainer.load_optim_state=true",
        "--train_module.validate_optimizer_hyperparameters_on_load=true",
        "--data_loader.restore_data_order_from_state=true",
    ...
    if missing:
        raise RuntimeError(f"Version-2 E16 spec missing: {missing}")
    ...
    if present:
        raise RuntimeError(f"same-batch Version-2 E16 continuation must not recalibrate: {present}")

#!/usr/bin/env bash
# DESCRIPTION:
#     - Runs one target-specific WSD endpoint for the standard 1B14B MoE on
#       the uniformly sampled 0802 unique 5B DCLM pool.
...
#       bash scripts/models/stdmoe_1b14b_step2_0802_unique_wsd.sh
...
runname="stdmoe_1b14b_step2_0802_unique_dclm5b_wsd_e${epochs}_lr${lr}_wd${wd}_warmup${warmup_steps}${run_suffix}"
...
launch src/scripts/train/olmoe-1B-7B_fsl.py "${runname}" \
		...
		--trainer.callbacks.wandb.tags="[pretraining, step2, 0802, standard-moe, unique-data, dclm-train-only, document-disjoint, uniform-document-sample, wsd, warmup24]" \

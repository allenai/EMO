...
repacking, and retains only E320 and E384.  Evaluations run as isolated WSD
branches from those retained checkpoints and never advance the producer.
"""
...
POLICY = "dense_small_pool3b_bs256_e384_continuation_v1"
...
def checkpoint_step(epoch: int) -> int:
    return producer.stable_step(
        epoch, producer.TARGET_POOL_TOKENS, 256
    )
...
def producer_arguments(config: dict[str, Any], item: dict[str, Any]) -> list[str]:
    source = Path(str(item["sourceCheckpoint"]))
    output = Path(str(item["output"]))
    target_steps = [checkpoint_step(epoch) for epoch in TARGET_EPOCHS]
    name = f"{EXPECTED_ID}-constant-pd-e256-e384-v1"
    return [
        *producer.base_arguments(item, str(config["repeatedManifest"])),
        ...
        (
            ...
            "pre-decay,e256-e384-continuation]"
        ),
...
def run_producer(config: dict[str, Any], item: dict[str, Any]) -> None:
    ...
    source = Path(str(item["sourceCheckpoint"]))
    targets = {
        epoch: output / f"step{checkpoint_step(epoch)}" for epoch in TARGET_EPOCHS
    }
    if all(checkpoint_complete(path) for path in targets.values()):
        print(
            f"DENSE_SMALL_POOL3B_PRODUCER_COMPLETE id={EXPECTED_ID} "
            f"epochs={list(TARGET_EPOCHS)}",
            flush=True,
        )
        return
    state_dir = output / ".constant_checkpoint_producer_pool3b_e256_e384_v1"
    producer.atomic_json(
        state_dir / "producer.json",
        {
            ...
            "output": str(output),
            "targetEpochs": list(TARGET_EPOCHS),
            "targetSteps": [checkpoint_step(epoch) for epoch in TARGET_EPOCHS],
            "evaluationEpochs": list(EVALUATION_EPOCHS),
    ...
    print(
        f"DENSE_SMALL_POOL3B_PRODUCER_START id={EXPECTED_ID} source={source} "
        f"epochs={list(TARGET_EPOCHS)} manifest={config['repeatedManifest']} "
    ...
    producer.run_torch(
        name,
        producer_arguments(config, item),
        state_dir / "producer.log",
    )
    missing = [epoch for epoch, path in targets.items() if not checkpoint_complete(path)]
    if missing:
        raise RuntimeError(f"continuation exited without complete checkpoints {missing}")
    producer.atomic_json(
        state_dir / "producer.json",
        {
            "policy": POLICY,

#!/usr/bin/env bash
# DESCRIPTION:
#     - Runs one target-specific WSD endpoint for the standard 1B14B MoE by
#       repeatedly cycling the independent uniformly sampled 0802 DCLM 1B pool.
#     - Uses 128 experts total: one always-active shared expert and top-7 routed
#       experts, for eight active experts per token.
#     - Later targets resume the preceding stable checkpoint from the same LR chain.
# USAGE:
#     EPOCHS=1 LR=5e-4 MODE=beaker \
#       bash scripts/models/stdmoe_1b14b_step2_0802_repeated_wsd.sh
#     START_FRESH=1 starts a newly introduced LR at a later endpoint.
##############################################################
set -eo pipefail
...
runname="stdmoe_1b14b_step2_0802_repeated_dclm1b_wsd_e${epochs}_lr${lr}_wd${wd}_warmup${warmup_steps}${run_suffix}"
...
launch src/scripts/train/olmoe-1B-7B_fsl.py "${runname}" \
		...
		--trainer.callbacks.wandb.tags="[pretraining, step2, 0802, standard-moe, repeated-data, dclm-train-only, document-disjoint, uniform-document-sample, wsd, warmup24]" \

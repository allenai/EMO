#!/usr/bin/env python3
"""Continue the 153M Pool-111M BS64 LR4e-3 WD0.3 trajectory to saturation."""
...
COORDINATE = "dense-153m-dclm111m-bs64-lr4e-3-wd0.3"
SOURCE_EPOCH = 32
TARGET_EPOCH = 128
NAME = f"{COORDINATE}-continuation-e32-saturation-e128-allocated-v1"
MIN_RUNTIME = "4h"
    ...
    record.update(
        {
            ...
            "continuationCheckpointEpochs": list(range(40, TARGET_EPOCH + 1, 8)),
            "continuationEvaluationEpochs": list(range(48, TARGET_EPOCH + 1, 16)),
            "continuationHardStopEpoch": TARGET_EPOCH,
            "submittedAt": datetime.now(tz=UTC).isoformat(),
            "minRuntime": spec["tasks"][0]["context"].get("minRuntime"),
        }
    )
...
def main() -> None:
    ...
    spec["description"] = (
        "Authorized 153M DCLM-111M BS64 LR4e-3 WD0.3 continuation from exact "
        "retained E32 PD. Save complete PD every 8 epochs, evaluate isolated "
        "uncapped 10% WSD POST every 16 epochs starting at E48, and stop at "
        "the first adjacent POST non-improvement or E128. One 4-H100 node, "
        "rank microbatch 16, GA1, canonical output, forced exact source, "
        f"auto-resume/eight retries, allocated with minRuntime={MIN_RUNTIME}."
    )
    if args.print_spec:

#!/usr/bin/env bash
# DESCRIPTION:
#     - Evaluates an exact 1B/2B/3B/4B/5B checkpoint from the unique-data run.
#     - Loads model weights without continuing training, evaluates at startup,
#       then cancels before taking an optimizer step.
# USAGE:
#     EPOCHS=1 CHECKPOINT_PATH=/weka/.../step239 MODE=beaker \
#       bash scripts/models/dense_1b_unique_checkpoint_eval.sh
# STATUS: EXPERIMENTAL
##############################################################
set -eo pipefail
...
checkpoint_path="${CHECKPOINT_PATH:?Set CHECKPOINT_PATH to the exact unique-data checkpoint}"
lr="${LR:-4e-4}"
wd="${WD:-0.033}"
...
runname="dense_1b_unique5b_${dataset_mix}_wsd_eval_e${epochs}_lr${lr}_wd${wd}_warmup${warmup_steps}"

launch src/scripts/train/olmo2-1B.py "${runname}" \
		--save-folder="${MODELS_DIR}/${runname}" \
		--dataset.mix="${dataset_mix}" \
		--work-dir="${DATASET_CACHE}" \
		--trainer.max_duration="{value: 5000000000, unit: tokens}" \
		--trainer.callbacks.wandb.enabled=true \
		--trainer.callbacks.wandb.entity=ai2-llm \
		--trainer.callbacks.wandb.project=sewonm-icsl \
		--trainer.callbacks.wandb.name="${runname}" \
		--trainer.callbacks.wandb.tags="[pretraining, unique-data, checkpoint-eval, dclm-full, wsd]" \

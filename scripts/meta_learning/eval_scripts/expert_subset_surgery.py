#!/usr/bin/env python3
# PARENT: "scripts/modular_extension/expert_subset_surgery.py"
"""Expert-subset surgery for the meta_learning phase-2 k=32 CPT (CPU, single process).

Identical to the modular_extension version except: the pool is a 128-expert model
(127 standard + 1 shared), the per-cluster selection comes from the ARM's own
expert_concentration.json (passed via --conc, since each arm clusters and selects with
its own router), and the per-stage bf16 pool snapshot is opt-in (--snapshot) -- at
~27GB x 32 stages x 2 arms it would strain the nearly-full weka volume.

extract   Build a (32 standard + 1 shared)-expert checkpoint from the 128-expert pool
          checkpoint, using a PER-LAYER selection of 32 standard experts. Subset slot
          order is the sorted original slot ids; the shared expert (pool slot 127)
          becomes subset slot 32 (last, as the router requires). Expert weights
          (w1/w2/w3), router rows, and Adam moments + per-tensor step are sliced
          identically (carry arm; --fresh-optim zeroes them).
...
Usage:
  python scripts/meta_learning/eval_scripts/expert_subset_surgery.py extract \\
      --pool <pool_dir> --out <subset_dir> --cluster 5 --conc <expert_concentration.json>
  python scripts/meta_learning/eval_scripts/expert_subset_surgery.py writeback \\
...
E_POOL = 128         # 127 standard + 1 shared (shared = last slot)
NS_POOL = 127
...
def slots_for_layer(selection, layer: int) -> list:
    """Pool slot ids that map to subset slots 0..32 for this layer (last = shared 127)."""
...
def load_selection_for_cluster(conc_path: str, cluster: int) -> dict:
...
def extract(args):
    ...
    if (out / "model_and_optim" / ".metadata").exists():
        ...
        return
    selection = load_selection_for_cluster(args.conc, args.cluster)
    ...
    if args.fresh_optim:
        ...
        # optimizer half is never read: skip loading the pool's moments entirely and
        # write a model-only checkpoint.
    ...
    with open(out / "selection.json", "w") as f:
        json.dump({"cluster": args.cluster, "num_experts": E_SUB, "num_shared": 1,
                   ...
                   "conc": str(args.conc), "slots_per_layer": selection}, f, indent=2)
...
def writeback(args):
    ...
    if args.snapshot:
        ...
        # per-stage eval / forgetting analysis). Off by default: weka space.
...
def main():
    ...
    pe.add_argument("--pool", required=True, help="128-expert pool step dir")
    pe.add_argument("--out", required=True, help="output subset dir")
    pe.add_argument("--cluster", type=int, required=True)
    pe.add_argument("--conc", required=True,
                    help="the arm's expert_concentration.json (per-cluster selection)")
    ...
    pw.add_argument("--snapshot", action="store_true",
                    help="also write a bf16 weights-only pool snapshot (~27GB/stage)")
    args = p.parse_args()

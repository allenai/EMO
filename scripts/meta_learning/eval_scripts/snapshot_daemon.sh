...
#
#   setsid nohup bash scripts/meta_learning/eval_scripts/snapshot_daemon.sh > <log> 2>&1 &
set -uo pipefail
...
while true; do
    for pool in "${POOLS[@]}"; do
        ...
        PYTHONPATH=src python scripts/meta_learning/eval_scripts/snapshot_pool.py --pool "$pool" --cluster "$c" \

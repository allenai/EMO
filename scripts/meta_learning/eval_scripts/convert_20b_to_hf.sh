...
#     pipeline (scripts/meta_learning/eval_scripts/launch_embed_docs.sh). Same flags as the
#     released-checkpoint conversions: fp32 weights, max seq len 4096, conversion on
#     CPU with GPU logit validation. Idempotent.
#
#   MODEL=sametok_ws_lam05 bash scripts/meta_learning/eval_scripts/convert_20b_to_hf.sh
#   MODEL=vanilla          bash scripts/meta_learning/eval_scripts/convert_20b_to_hf.sh
set -euo pipefail

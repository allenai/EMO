...
#
#   bash scripts/meta_learning/eval_scripts/run_snapshot_evals.sh
set -uo pipefail
    ...
    while [ "$inflight" -lt "$MAX_INFLIGHT" ]; do
        ...
        for arm in "${ARMS[@]}"; do
            ...
            if MODEL="$arm" \
               ...
               bash scripts/meta_learning/eval_scripts/launch_eval_k32cpt.sh > "${EV}/launch_${tag}.log" 2>&1; then

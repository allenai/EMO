...
#
#   MODEL=sametok_ws_lam05 bash scripts/meta_learning/eval_scripts/run_k32cpt_arm.sh
#   MODEL=vanilla          bash scripts/meta_learning/eval_scripts/run_k32cpt_arm.sh
...
for c in $CLUSTERS; do
    ...
    submit_stage() {  # retries around the flaky GitPython/launcher path; sets exp_id
        ...
        for attempt in 1 2 3 4 5; do
            MODE=beaker MODEL=$MODEL CLUSTER=$c \
                ...
                TOKENS=$tokens \
                bash scripts/meta_learning/eval_scripts/k32cpt_stage.sh 2>&1 | tee "${RUNS}/launch_${tag}.log" || true
    ...
    OPENBLAS_NUM_THREADS=16 PYTHONPATH=src python scripts/meta_learning/eval_scripts/expert_subset_surgery.py writeback \

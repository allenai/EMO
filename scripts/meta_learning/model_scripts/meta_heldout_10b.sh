# PARENT: "scripts/meta_learning/model_scripts/meta_sametok_10b.sh"
# DESCRIPTION:
#     meta_learning pilot arm (iii): FOMAML with a held-out outer split. Identical to the
#     same-tokens arm EXCEPT meta_mode=heldout: each rank's micro-batches are split in half —
#     inner pass (selective top-32 + pseudo-step) on the first half, outer full-routing pass on
#     the second half. The meta-gradient therefore rewards inner updates that GENERALIZE rather
#     than one-step memorization (standard MAML support/query). Per-step compute ~= the vanilla
#     baseline, but the real (outer) update only sees half the tokens per step — the effective
#     outer batch is 512 instances, documented caveat vs the other arms.
#
#     Requires an even number of micro-batches per rank: 1024 instances / 64 ranks = 16 per
#     rank, rank_microbatch = 4 instances -> 4 micro-batches. OK.
#
#   git add . && git commit && git push origin <branch>   # gantry clones from origin!
#   MODE=beaker bash scripts/meta_learning/model_scripts/meta_heldout_10b.sh
##############################################################
...
launch src/scripts/train/olmoe-1B-7B_fsl_meta.py $runname \
		...
		--trainer.callbacks.wandb.tags="[pretraining, ${EXPERIMENT_NAME}, heldout]" \

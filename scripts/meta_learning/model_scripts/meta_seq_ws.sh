# PARENT: "scripts/meta_learning/model_scripts/meta_seq_ws.sh"
# DESCRIPTION:
#     meta_learning arm (vii): SEQUENTIAL ablation of same-tokens FOMAML, 20B tokens. Per token
#     batch, TWO real AdamW steps instead of a temporary probe: (1) an ordinary selective step
#     (per-doc top-32 routing; router/attention/experts all update; LB/z on), then (2) a full-
#     routing step on the SAME tokens at the post-step weights, expert grads masked to each
#     doc's working set. Isolates what the FOMAML probe adds beyond plain sequential training:
#     vs sametok_ws_lam05 the selective step here is COMMITTED and AdamW-shaped (no large
#     transient raw-SGD probe, no evaluation-at-theta'). NOTE: 2x optimizer updates per token
#     (9,536 over 20B) though the trainer counts 4,768 steps like the other arms; inner_lr
#     unused.
...
launch src/scripts/train/olmoe-1B-7B_fsl_meta.py $runname \
		...
		--trainer.callbacks.wandb.tags="[pretraining, ${EXPERIMENT_NAME}, seq_ws]" \
		...
		--train_module.meta_mode=sequential \

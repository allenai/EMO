...
# DESCRIPTION:
#     meta_learning REDESIGN arm, SCALE-ISOLATION variant. Identical to the adam arm EXCEPT:
#       - inner_lr_scale = [1, 1]   -- NO magnitude band. theta' is EXACTLY the true "next
#                                      selective AdamW step" the real optimizer would take
#                                      (base step size = live scheduler lr). This isolates the
#                                      (1) Adam-preconditioning + match_lr mechanism from the
#                                      (2) magnitude randomization, after the [1,32]-band arm
#                                      regressed full CE by +0.30 (aggressive displacements
#                                      damaged base quality). If this recovers full CE toward
#                                      the SGD/vanilla level while keeping the improved
#                                      selective-vs-full gap, the Adam pseudo-step is the win.
...
#     Everything else IDENTICAL to meta_sametok_ws_lam05_adam.sh.
#
#   git add . && git commit && git push origin <branch>   # gantry clones from origin!
#   MODE=beaker PREEMPTIBLE=0 bash scripts/meta_learning/model_scripts/meta_sametok_ws_lam05_adam_scale1.sh
##############################################################
...
inner_lr_scale_max=1

runname="meta128_sametok_ws_lam05_adam_scale1"
...
launch src/scripts/train/olmoe-1B-7B_fsl_meta.py $runname \
		...
		--trainer.callbacks.wandb.tags="[pretraining, ${EXPERIMENT_NAME}, same_tokens_ws_lam05_adam_scale1]" \

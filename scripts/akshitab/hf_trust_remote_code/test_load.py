...
def main() -> None:
    ...
    p.add_argument("--repo-id", required=True, help="HF repo id, or path to a local checkpoint dir.")
    ...
    p.add_argument("--token", default=None, help="HF token for private repos. Defaults to cached login / HF_TOKEN.")
    args = p.parse_args()

    dtype = {"float32": torch.float32, "float16": torch.float16, "bfloat16": torch.bfloat16}[args.dtype]

    print(f"loading tokenizer: {args.repo_id}")
    tokenizer = AutoTokenizer.from_pretrained(args.repo_id, trust_remote_code=True, token=args.token)

#!/bin/bash
# Token-level focused sweep v2: follow-up from token v1 results.
#
# v1 found: token_probs/l2/k=32 was best (sil_cos=0.1287).
# v2 tests whether PCA dim-reduction and spherical k-means can improve further,
# mirroring the document-level v2 sweep that improved sil_cos from 0.2287 to 0.2490.
#
# Grid:
#   Embedding:  token_probs
#   Transforms: mean_pca_l2
#   Clustering: kmeans, spherical_kmeans
#   k:          16, 32, 64, 128, 256, 512
#
# = 1 embedding × 1 transform × 2 clusterers × 6 k-values = 12 runs
#
# Usage:
#   bash scripts/ryanwang/analysis/run_sweep_focused_token_v2.sh <DATA_DIR>
#
# Example:
#   bash scripts/ryanwang/analysis/run_sweep_focused_token_v2.sh \
...
DATA_DIR="$1"
OUTPUT_TSV="${DATA_DIR}/sweep_results_focused_token_v2.tsv"

EMBEDDINGS="token_probs"
TRANSFORMS="mean_pca_l2"
CLUSTERS="kmeans spherical_kmeans"
...
for CLUST in $CLUSTERS; do
    ...
    for K in $K_VALUES; do
        ...
        echo -e "${EMB}\t${TRANS}\t${CLUST}\t${K}\t${SIL:-NA}\t${SIL_COS:-NA}\t${CH:-NA}\t${DB:-NA}\t${SZ_MIN:-NA}\t${SZ_MAX:-NA}\t${SZ_MED:-NA}\t${SZ_STD:-NA}\t${SRC_ENT:-NA}" >> "$OUTPUT_TSV"

        echo "  [$COUNT] $EMB / $TRANS / $CLUST / k=$K  sil_euc=${SIL:-NA}  sil_cos=${SIL_COS:-NA}"
    done
...
echo ""
echo "================================================================"
echo "  TOKEN SWEEP v2 COMPLETE: $COUNT runs"

...
from src.scripts.clustering.utils import load_model_and_tokenizer, get_moe_config, EOS_TOKEN_ID
...
def main():
    ...
    for bs in BATCH_SIZES:
        ...
        try:
            ...
            peak_mem = max(torch.cuda.max_memory_allocated(i) / 1e9
                          for i in range(torch.cuda.device_count()))
            results.append((bs, avg, std, tok_s, peak_mem))
            logger.info(f"  {avg:.3f}s ± {std:.3f}s per batch | "
    ...
    # Summary
    logger.info("\n" + "=" * 80)
    logger.info(f"{'BS':>5}  {'Time/batch':>12}  {'Tok/s':>12}  {'Peak GPU MB':>12}  {'ETA 20M tok':>12}")
    logger.info("-" * 80)
    for bs, avg, std, tok_s, peak in results:
        eta_min = 20_000_000 / tok_s / 60
        logger.info(f"{bs:>5}  {avg:>9.3f}s  {tok_s:>12,.0f}  {peak*1024:>10,.0f}  {eta_min:>9.1f} min")
    logger.info("=" * 80)

...
# DESCRIPTION:
#     - Same as _ct-m8_lb0 (lb=0) continual-pretrain, but with everything except the
#       expert MLPs frozen via --model.freeze_params.
#     - Frozen: embeddings, attention (all), feed_forward_norm, router, lm_head.
#     - Trainable: only blocks.*.feed_forward_moe.experts.* (all 128 expert MLPs).
...
runname="twolevelbatchlbreducedp512sharedexp1randpool-8-128eval32_1b14b_lr-4e-3_lb-1e-1_1T_0313_anneal_from_step23_ct-m8_lb0_frz"
...
python -m olmo_core.launch.beaker \
  ...
  --shared-filesystem \
    ...
    $runname \
		...
		--trainer.callbacks.wandb="{enabled: true, entity: ryanyxw, project: olmoe-modular, name: ${runname}, tags: [extension, contpretrain, finemath, lb0, frz]}" \

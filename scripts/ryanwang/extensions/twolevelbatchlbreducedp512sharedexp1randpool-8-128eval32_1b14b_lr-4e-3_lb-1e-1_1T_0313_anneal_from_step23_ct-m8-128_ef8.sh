...
# DESCRIPTION:
#     - Same as _ct-math_8-128 but with extension_finetune_mode active (top_e=8).
#     - Per-doc top-8 expert MLPs train; non-top-8 expert MLPs and the shared expert
#       are detached (no grad). Router is left fully trainable (CE/LB/z all flow).
#     - Note: runname drops trailing "8419" from step238419 to fit the 128-char
...
# extension_finetune_* flags: top_e=8 expert MLPs train per doc; router untouched
ef_mode=true
ef_top_e=8
ef_detach_router=false
...
runname="twolevelbatchlbreducedp512sharedexp1randpool-8-128eval32_1b14b_lr-4e-3_lb-1e-1_1T_0313_anneal_from_step23_ct-m8-128_ef8"
...
python -m olmo_core.launch.beaker \
  ...
  --shared-filesystem \
    ...
    $runname \
		...
		--trainer.callbacks.wandb="{enabled: true, entity: ryanyxw, project: olmoe-modular, name: ${runname}, tags: [extension, contpretrain, finemath, top-e8]}" \

...
model_name="twolevelbatchlb-32_1b14b_stability_filter-true_zlossweight-1e-3_1115"
#model_name="moe_1b7b_olmoe-mix"
step="step30995"
prune_keep_k=32
num_checkpoints=5

ABLATION_NAME="-lr_2e-5"
...
for train_task_name in "${train_task_names[@]}"; do
    ...
    runname="${model_name}_${step}_finetune_${task_prefix}_keepk${prune_keep_k}${ABLATION_NAME}"
    # limit runname to 128 characters from the end
    runname=$(echo $runname | rev | cut -c1-100 | rev)
#    runname=$(echo $runname | cut -c1-100)


    out_dir="${task_prefix}_finetune-keepk${prune_keep_k}${ABLATION_NAME}"
...
#        --model.block.feed_forward_moe.num_experts=128 \
    ...
    python -m olmo_core.launch.beaker \
      ...
      -- src/scripts/train/olmoe-1B-7B_finetune_ablate.py \
        $runname \

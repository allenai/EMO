...
# ---- 4. collect ----
WALL=("${W1[@]}" "${W2[@]}"); [ $MAXW -ge 3 ] && WALL+=("${W3[@]}")
expected=""; for s in "${WALL[@]}"; do expected="$expected $HR/merged_match$s"; for g in $GS; do expected="$expected $HR/sub${g}_match$s"; done; done
for tag in $expected; do until have $S/olmoe3_routing/$tag/none; do sleep 300; done; done

...
V="${1:?std_k8|s128_k4|s128_k8|emo_k8}"; S=sparse_experts; W=/weka/oe-training-default/ryanwang/EMO/sparse_experts; SAMPLE=sample_8k_300b.npz; SRCSTD=$S/olmoe3_squares_std
EMO=0; MAXW=3   # EMO loss on the squares; number of windows to train (1 = stop after 10B -> 20B)
case $V in
  ...
  *) echo "std_k8|s128_k4|s128_k8|emo_k8"; exit 1;;
esac
...
if [ -f $S/$PACKS/groups.json ] && [ "$PACKS" != build ] && [ "$E" = 512 ]; then [ -f $SQ/groups.json ] || cp $S/$PACKS/groups.json $SQ/groups.json; fi   # same random groups as the arm whose packs we share (same expert count)
...
for g in $GS; do S1[$g]=$(steps_of $SQ/pack/stats.json $g)
  launch_train $LOG/square${g}_launched $LOG/launch_square$g.log scripts/sparse_experts/model_scripts/olmoe3_275m_emo_square.sh SQUARE_GROUP=$g SQUARES_NAME=$SQN OLMOE3_EMO=$EMO OLMOE3_NUM_EXPERTS=$E OLMOE3_RUNNAME=${RP}$g OLMOE3_WANDB_TAGS=$SQN,square
done
...
if [ $MAXW = 1 ]; then
  expected=""; for s in "${W1[@]}"; do expected="$expected $HR/merged_match$s"; for g in $GS; do expected="$expected $HR/sub${g}_match$s"; done; done
...
for g in $GS; do st=(${S1[$g]}); S2[$g]=$(steps_of $SQ/pack2/stats.json $g)
  ...
  launch_train $LOG/square${g}_w2_launched $LOG/launch_square${g}_w2.log scripts/sparse_experts/model_scripts/olmoe3_275m_emo_square.sh SQUARE_GROUP=$g SQUARES_NAME=$SQN OLMOE3_EMO=$EMO OLMOE3_NUM_EXPERTS=$E OLMOE3_TOKENS=$tokens OLMOE3_DATA_PATHS="$W/$SQN/pack2/group$g/*.npy" OLMOE3_INIT_FROM="$W/$SQN/init2/group$g/model_and_optim" OLMOE3_RUNNAME=${RP}${g}_w2 OLMOE3_WANDB_TAGS=$SQN,square,w2
done
...
for g in $GS; do st=(${S2[$g]}); start=$((B0 + g * SQ3))
  ...
  launch_train $LOG/square${g}_w3_launched $LOG/launch_square${g}_w3.log scripts/sparse_experts/model_scripts/olmoe3_275m_stdrand_square_w3.sh SQUARE_GROUP=$g SQUARES_NAME=$SQN RUN_PREFIX=$RP OLMOE3_EMO=$EMO OLMOE3_NUM_EXPERTS=$E FT_START=$W/$SQN/ft_start/w3_group$g FT_START_STEP=$start FT_STEPS=$SQ3
done

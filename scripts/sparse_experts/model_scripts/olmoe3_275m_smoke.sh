...
#     (~21M tokens) so the whole stack runs once (B300 image, pinned OLMo-core install, Dolma 3.5
#     index build from S3, KDA + FA4 kernels, fused MoE-v2 blocks, checkpoint to weka, W&B).

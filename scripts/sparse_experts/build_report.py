...
def build_method(an: dict) -> str:
    ...
    stats = (
        ...
        "activates 7 of the 511 standard experts. Expert A is active on 20,000 tokens, expert B on "
        "10,000, and both together on 1,000.</p>"
        "<ul>"
        "<li><b>Usage</b> p<sub>i</sub> = C<sub>ii</sub>/N: the share of tokens an expert is active on. "
        "A: 0.02, B: 0.01; perfectly even usage would put every expert at 7/511 &asymp; 0.0137. "
        "<b>Effective #experts</b> = exp(entropy of the usage distribution): 511 if all experts were used "
        "equally, 100 if only 100 were used equally; ~420 here means broad but uneven use. "
        "<b>Gini</b>: 0 = perfectly even, 1 = one expert takes everything.</li>"
        ...
        "describe this distribution over all ~130k pairs: a median below 1 with p99 near 10 means most "
        "pairs are slightly below chance while a small tail is strongly bound.</li>"
        ...
        "&asymp; 7/511 = 0.014 for every pair.</li>"
        "<li><b>Pairs never co-active</b>: the share of pairs with C<sub>ij</sub> = 0.</li>"
        ...
        "construction under the pool-64 pass).</li>"

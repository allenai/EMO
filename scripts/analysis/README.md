...
```bash
bash scripts/analysis/run_analyze_data_mix.sh
```
...
```bash
# Default model
bash scripts/analysis/run_extract_embeddings.sh

# Specific model
bash scripts/analysis/run_extract_embeddings.sh models/<model_name>/step<N>-hf
```
...
bash scripts/analysis/run_extract_embeddings_shuffled_token.sh models/<model_name>/step<N>-hf

# Token-level extraction with custom token count (e.g. 1M tokens)
bash scripts/analysis/run_extract_embeddings_shuffled_token.sh models/<model_name>/step<N>-hf 1000000
...
bash scripts/analysis/run_extract_embeddings_shuffled_token_truncated.sh models/<model_name>/step<N>-hf 1000000 100
```
...
```bash
# Default model
bash scripts/analysis/run_extract_embeddings_mmlu.sh

# Specific model
bash scripts/analysis/run_extract_embeddings_mmlu.sh models/<model_name>/step<N>-hf
```
...
```bash
# Default model
bash scripts/analysis/run_extract_embeddings_hellaswag.sh

# Specific model
bash scripts/analysis/run_extract_embeddings_hellaswag.sh models/<model_name>/step<N>-hf
```
...
- `scripts/analysis/split_hellaswag_train_val.py` — train+val split (also called by the shell wrapper)
...
# Single run
bash scripts/analysis/run_transform_and_cluster.sh "$DATA_DIR" probs mean_pca_l2 spherical_kmeans 32

# Single run with saving (writes assignments.npy, summary.json, run_info.json)
bash scripts/analysis/run_transform_and_cluster.sh "$DATA_DIR" probs mean_pca_l2 spherical_kmeans 32 --save
```
...
```bash
# Default model
bash scripts/analysis/run_expert_coverage.sh

# Specific model
bash scripts/analysis/run_expert_coverage.sh models/<model_name>/step<N>-hf
```

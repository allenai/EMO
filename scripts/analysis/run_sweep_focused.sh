#!/bin/bash
# Focused sweep: probs + topk_freq × {identity, l2} × kmeans × k∈{32,64,128}
#
# Reports silhouette (euclidean), silhouette (cosine), calinski-harabasz,
# davies-bouldin, cluster size stats, and avg source entropy.
#
# Usage:
#   bash scripts/analysis/run_sweep_focused.sh <DATA_DIR>
#
# Example:
#   bash scripts/analysis/run_sweep_focused.sh \
#       claude_outputs/analysis/router_clustering_pretraining_shuffled/twolevelbatchlbreducedp512sharedexp1randpool-8-128eval32_1b14b_lr-4e-3_lb-1e-1_0301
set -e
...
DATA_DIR="$1"
OUTPUT_TSV="${DATA_DIR}/sweep_results_focused.tsv"

EMBEDDINGS="probs topk_freq"

(() => {
  ...
  if(gridMount){
    ...
    gridMount.innerHTML=[...groups.entries()].sort((a,b)=>{const [ae,ak]=a[0].split('|'),[be,bk]=b[0].split('|');return Number(ae)-Number(be)||ak.localeCompare(bk)}).map(([g,runs])=>{const [epoch,label]=g.split('|');const ordered=runs.sort((a,b)=>Number(a.lr)-Number(b.lr));const complete=ordered.filter(r=>r.status==='complete'&&Number.isFinite(r.validation??r.c4));const pending=ordered.some(r=>['active','queued'].includes(r.status));const selected=!pending&&complete.length>=2?complete.reduce((best,r)=>(r.validation??r.c4)<(best.validation??best.c4)?r:best):null;const chips=ordered.map(r=>`<span class="tuple ${selected===r?'selected':['active','queued','planned'].includes(r.status)?'active':''}">${r.lr||'—'} · ${r.status}${Number.isFinite(r.validation??r.c4)?` · ${(r.validation??r.c4).toFixed(3)}`:''}</span>`).join('');return `<tr><td><strong>E${epoch}</strong></td><td>${label}</td><td><div class="tuple-list">${chips}</div></td><td>${selected?`<strong>${selected.lr}</strong> · DCLM validation CE ${(selected.validation??selected.c4).toFixed(3)}`:'Pending LR comparison'}</td></tr>`;}).join('');
  }
})();

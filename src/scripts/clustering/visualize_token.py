"""
Interactive HTML cluster explorer for token-level clusterings.

4 views (Clusters / Documents / Tokens / UMAP) plus a per-document reader modal
with per-token cluster highlighting.

Usage:
    python -m src.scripts.clustering.visualize_token \
        --cluster-dir claude_outputs/clustering/pretraining/<model>/probs_mean_pca_l2_spherical_kmeans_k32 \
        --context-window 10

Reads from the cluster-dir and (auto-detected) parent data-dir:
    <cluster-dir>/assignments.npy, summary.json, run_info.json
    <data-dir>/embeddings_<name>.npy, documents.npy, doc_boundaries.npy,
               metadata_tokens.jsonl.gz, info.json

Writes cluster_explorer.html into the cluster-dir. Reads optional
cluster_labels.json if present to use human labels instead of "Cluster N".
"""
...
def main():
    ...
    for c in summary:
        # New clustering pipeline uses "representative_samples"; fall back to old key.
        rep_list = c.get("representative_samples") or c.get("representative_docs")
        if rep_list is None:
            continue
        c["representative_docs"] = rep_list  # normalize for downstream code
        for rd in rep_list:
            idx = rd.get("idx")
    ...
    prep_key = run_info.get("preprocess", run_info.get("transform", "unknown"))
    method_key = run_info.get("method", run_info.get("cluster", "unknown"))
    emb_label = f"{run_info['embedding']}_{prep_key}_{method_key}"

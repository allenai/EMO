"""
Train selected experts by index, with the router frozen.

Unlike train_selected_experts.py, this script also freezes the router weights
by adding them to freeze_params and removing "router" from gradient mask patterns.

Launch this with torchrun:

    torchrun --nproc-per-node=4 src/scripts/akshitab/add_finegrained_expert/train_selected_experts_no_router.py run_name \
...
class SelectedExpertGradientMaskCallback(Callback):
    ...
    layer_patterns: List[str] = field(default_factory=lambda: ["experts"])
...
def parser_args():
    parser = argparse.ArgumentParser(
        ...
        description="Train selected experts (by index) in an MoE model, with router frozen.",

"""
Train new expert(s) in an MoE model with a frozen shared expert.

The shared expert is expected to already be present in the checkpoint
(produced by add_shared_expert.py). Its weights are frozen during training.

Launch this with torchrun:

    torchrun --nproc-per-node=4 src/scripts/akshitab/add_finegrained_expert/train_new_expert_shared.py run_name [OVERRIDES...]
"""
...
def build_config(opts, overrides: List[str]) -> ExperimentConfig:
    ...
    model_config = TransformerConfig.olmoe_1B_7B_shared_expert(
        ...
        freeze_params=[
            ...
            "blocks.*.feed_forward_moe.shared_mlp.*",
            "lm_head.*",
        ],
    )
...
def parser_args():
    parser = argparse.ArgumentParser(
        ...
        description="Train a transformer language model with shared expert.",

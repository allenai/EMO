"""
Train new expert(s) in an MoE model with certain experts kept always active.

Always-active experts are included in every token's top-k selection regardless of
router scores. They consume slots from top_k: if top_k=8 and 2 experts are always
active, the router dynamically selects 6, and the 2 always-active fill the rest.

Launch this with torchrun:

    torchrun --nproc-per-node=4 src/scripts/akshitab/add_finegrained_expert/train_new_expert_always_active.py run_name \
        --always-active-experts 128 129 [OVERRIDES...]
"""
...
def build_config(opts, overrides: List[str]) -> ExperimentConfig:
    ...
    always_active_experts = opts.always_active_experts if opts.always_active_experts else None
    ...
    if always_active_experts:
        log.info(f"Always-active experts: {always_active_experts}")
...
def parser_args():
    parser = argparse.ArgumentParser(
        ...
        description="Train new expert(s) in an MoE model with always-active experts.",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Expert indices to keep always active (e.g., --always-active-experts 128 129). "
        "These experts are included in every token's top-k, consuming slots from the top_k budget.",
    )
    parser.add_argument(
        "--eval-only",
        action="store_true",

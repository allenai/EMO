...
def main():
    ...
    for label, stats in source_stats.items():
        stats["fraction"] = stats["est_tokens"] / grand_total_tokens if grand_total_tokens > 0 else 0.0
    ...
    print("=" * 90)
    print(f"{'TOTAL':<45} {len(all_paths):>6} {grand_total_tokens:>14,} {'100.00%':>9} {'500,000,000':>12}")
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"{'SOURCE':<45} {'FILES':>6} {'EST_TOKENS':>14} {'FRACTION':>9} {'500M_ALLOC':>12}\n")
        f.write("-" * 90 + "\n")
        for label, stats in sorted_sources:
            f.write(
                f"{label:<45} {stats['num_files']:>6} {stats['est_tokens']:>14,} "

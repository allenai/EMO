...
def build_config(opts, overrides: List[str]) -> ExperimentConfig:
    # Load trainer state from checkpoint to get the current step
    trainer_state_path = os.path.join(opts.anneal_checkpoint, "train", "rank0.pt")
    log.info(f"Loading trainer state from {trainer_state_path} to determine global step...")
    ...
    anneal_steps = opts.anneal_tokens // batch_size_in_tokens
    log.info(f"Annealing for {anneal_steps} steps ({opts.anneal_tokens / 1e9:.1f}B tokens)")
    ...
    config = ExperimentConfig(
        ...
        load_path=opts.anneal_checkpoint,
    ...
    # Set max_duration for annealing: run until checkpoint_step + anneal_steps
    config.trainer.max_duration = Duration.steps(checkpoint_step + anneal_steps)
...
def parser_args():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Token budget for the anneal phase (e.g. 100_000_000_000 for 100B tokens).",
    )
    parser.add_argument(
        "--anneal-checkpoint",
        type=str,
        required=True,
        help="Path to the checkpoint to resume from for annealing.",
    )

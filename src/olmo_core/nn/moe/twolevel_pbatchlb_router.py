            ...
            with torch.autocast(enabled=False, device_type=x.device.type):
                ...
                if self.expert_uncond_lb_prob_bias is not None:
                    if expert_probs is None:
                        ...
                        # first get expert probability usage
                        expert_probs = torch.mean(valid_scores, dim=0)
                    # get loss by multiplying distribution with detached copy of distribution
                    expert_uncond_lb_prob = torch.dot(expert_probs, expert_probs.detach())
...
class MoETwoLevelPBatchLBRouterConfig(MoETwoLevelRouterConfig):
    ...
    def build(
        ...
        *,
        ...
        return MoETwoLevelPBatchLBRouter(**kwargs)

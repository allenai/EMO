...
class MoETwoLevelBatchLBReduceDPSharedExpRandPoolGhostRouter(
    MoETwoLevelBatchLBReduceDPSharedExpRandPoolRouter
):
    """
    The ``models_fullextend`` "ghost expert" variant of
    :class:`MoETwoLevelBatchLBReduceDPSharedExpRandPoolRouter`.

    The base (published) randpool router is left completely untouched; this subclass adds two
    experimental, opt-in behaviors and overrides :meth:`forward` to host them:

    1. **Ghost-expert training** (``ghost_extend_mode``). For each document a "ghost" expert is
       simulated as a linear combination of the document pool's expert weights; the model is
       trained with this perpetually-blended new expert present so that, at the end of training,
       instantiating and adding a real new expert is well-conditioned. The ghost's router row is
       the same alpha-blend of the pool's router rows, so its logit joins the routing softmax
       denominator (the real pool experts shrink to make room). Ghosts are added only in
       ``train()`` mode; eval/inference measure the base model with no ghost, so eval behavior is
       identical to the pristine parent. The per-document blend specs are stashed in
       ``self._ghost_extend_stash`` and consumed by ``MoEBase.forward``.

    2. **New-expert activation tracking** (``num_new_experts``). When ``>0``, the router logs
       (metric-only) what fraction of tokens / documents per batch route to the last
       ``num_new_experts`` non-shared experts (used by the add-a-real-expert extension eval).

    With ``ghost_extend_mode=False`` and ``num_new_experts=0`` this router is behaviorally
    identical to its parent (the ``forward`` reduces to the same computation).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # All pristine randpool params (min/max/eval pool, eos_token_id, num_shared_experts,
        # num_forced_experts, extension_finetune_*, dtype, init_device, ...) flow through to the
        ...
        # in each batch route to (any of) these new experts. Default 0 => no-op.
        self.num_new_experts = num_new_experts
        self._new_expert_token_activation = 0.0
        self._new_expert_doc_activation = 0.0

        # Ghost-expert training (models_fullextend). See the class docstring.
    ...
    def forward(
        ...
        **kwargs,
        ...
        with random document expert pool sizes, plus the ghost-expert and new-expert-activation
        behaviors described in the class docstring.
        """
    ...
    def extra_repr(self):
        """Append the ghost / new-expert knobs to the parent (pristine) repr."""
        base_repr = super().extra_repr()
        return (
            f"{base_repr}, num_new_experts={self.num_new_experts}, "
            f"ghost_extend_mode={self.ghost_extend_mode}, ghost_extend_num={self.ghost_extend_num}, "
            f"ghost_extend_coeff_mode={self.ghost_extend_coeff_mode}, "
            f"ghost_extend_random_k={self.ghost_extend_random_k}, "
            f"ghost_extend_route={self.ghost_extend_route}, "
            f"ghost_extend_detach_coeff={self.ghost_extend_detach_coeff}"
        )


@dataclass
class MoETwoLevelBatchLBReduceDPSharedExpRandPoolGhostRouterConfig(
    MoETwoLevelBatchLBReduceDPSharedExpRandPoolRouterConfig
):
    # Inherits the pristine randpool fields (num_shared_experts, min/max/eval pool,
    # num_forced_experts, extension_finetune_*) and adds the models_fullextend extras.
    num_new_experts: int = (
        0  # last N non-shared experts are "new" (metric-only: log token/doc activation)
    )
    ...
    def build(
        ...
        *,
        ...
        init_device: str = "cpu",
    ) -> MoETwoLevelBatchLBReduceDPSharedExpRandPoolGhostRouter:
        """
        Build the router.
        """
        ...
        return MoETwoLevelBatchLBReduceDPSharedExpRandPoolGhostRouter(**kwargs)

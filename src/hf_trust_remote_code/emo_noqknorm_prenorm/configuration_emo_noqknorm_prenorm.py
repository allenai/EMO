...
class EmoNoQKNormPrenormConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`EmoNoQKNormPrenormModel`]. It is used to instantiate an EmoNoQKNormPrenorm
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the [allenai/EmoNoQKNormPrenorm-7x7B-1T](https://huggingface.co/allenai/EmoNoQKNormPrenorm-7x7B-1T).
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 100352):
            Vocabulary size of the EmoNoQKNormPrenorm model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`EmoNoQKNormPrenormModel`]
    ...
    ```python
    >>> from transformers import EmoNoQKNormPrenormModel, EmoNoQKNormPrenormConfig

    >>> # Initializing a EmoNoQKNormPrenorm style configuration
    >>> configuration = EmoNoQKNormPrenormConfig()

    >>> # Initializing a model from the EmoNoQKNormPrenorm style configuration
    >>> model = EmoNoQKNormPrenormModel(configuration)
...
__all__ = ["EmoNoQKNormPrenormConfig"]

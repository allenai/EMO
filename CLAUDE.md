...
`bash scripts/publish_reports.sh` rebuilds every registered experiment report (`scripts/<experiment>/build_report.py` → `claude_outputs/<experiment>/report.html`) and deploys them to Cloudflare Pages at `https://emo-reports.pages.dev/` (index page plus one page per experiment; direct upload, cache invalidates in seconds). New experiments register with one `name|blurb` line in the script. Credentials live in `/root/.config/cloudflare/emo_reports.env` (never commit them; this repo is public); wrangler runs from the Node install at `/root/.local/node`, both persisted on weka. The deploy step pushes repo-derived content to an external host, so the user must run it (or explicitly approve it) rather than Claude deploying autonomously. An earlier gist-based publish flow lives at the (deprecated) gist whose ID is in the untracked `claude_outputs/.report_gist_id`.
